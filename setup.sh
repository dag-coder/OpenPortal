...
DATABASE_URL=postgres://openproxy:${DB_PASSWORD}@${DB_HOST}:5432/openproxy?sslmode=disable
POSTGRES_USER=openproxy
POSTGRES_PASSWORD=${DB_PASSWORD}
POSTGRES_DB=openproxy
...
FILTEREOF
  ...
  cat > /etc/fail2ban/jail.d/openproxy.conf << JAILEOF
[openportal-auth]
enabled  = true
...
findtime = 600
bantime  = 1800
action   = iptables-multiport[name=openproxy, port="80,443,${PORT_API}"]
JAILEOF
...
setup_unattended_upgrades() {
  ...
  case "$mgr" in
    apt)
      ...
      cat > /etc/apt/apt.conf.d/50openproxy-auto-upgrades << 'UPGEOF'
...
harden_sysctl() {
  cat > /etc/sysctl.d/99-openproxy-hardening.conf << 'SYSCTLEOF'
...
SYSCTLEOF
  sysctl -p /etc/sysctl.d/99-openproxy-hardening.conf &>/dev/null || true
...
setup_bare_metal() {
  ...
  sudo -u postgres psql -c \
    "CREATE USER openproxy WITH PASSWORD '${DB_PASSWORD}';" 2>/dev/null || \
  sudo -u postgres psql -c \
    "ALTER USER openproxy WITH PASSWORD '${DB_PASSWORD}';" 2>/dev/null || true
  sudo -u postgres psql -c \
    "CREATE DATABASE openproxy OWNER openproxy;" 2>/dev/null || \
    warn "Database already exists"
  info "PostgreSQL: database 'openproxy' ready"

  # ── Service user ────────────────────────────────────────────────────────────
  step "Creating service user"
  useradd --system --home-dir /opt/openportal --create-home \
    --shell /bin/false openproxy 2>/dev/null || true
  info "User: openproxy"
  ...
  CGO_ENABLED=0 GOPATH=/tmp/go-cache "$GO_BIN" build \
    -ldflags="-s -w" -o /opt/openportal/openproxy ./cmd/server
  cp -r migrations/ /opt/openportal/
  info "Binary: /opt/openportal/openproxy"
  ...
  chown openproxy:openproxy /opt/openportal/.env
...
WGEOF
  ...
  echo "net.ipv4.ip_forward=1" > /etc/sysctl.d/99-openproxy-wg.conf
  sysctl -p /etc/sysctl.d/99-openproxy-wg.conf &>/dev/null || true
  ...
  local nginx_conf="/etc/nginx/sites-available/openproxy"
...
NGINXEOF
  ...
  if [[ -d /etc/nginx/sites-enabled ]]; then
    ln -sf "$nginx_conf" /etc/nginx/sites-enabled/openproxy
    rm -f /etc/nginx/sites-enabled/default
  else
    cp "$nginx_conf" /etc/nginx/conf.d/openproxy.conf
  ...
  systemctl enable --now openproxy
  info "Service: openproxy (systemd)"

  chown -R openproxy:openproxy /opt/openportal
...
setup_dev() {
  ...
  if pg_isready -q 2>/dev/null; then
    ...
    if [[ -n "$pgcmd" ]]; then
      $pgcmd psql -c "CREATE USER openproxy WITH PASSWORD '${DB_PASSWORD}';" 2>/dev/null || \
      $pgcmd psql -c "ALTER USER openproxy WITH PASSWORD '${DB_PASSWORD}';"  2>/dev/null || true
      $pgcmd psql -c "CREATE DATABASE openproxy OWNER openproxy;" 2>/dev/null || \
        warn "Database already exists — skipping"
      info "Database 'openproxy' ready"
    else
      # Try as current user
      createuser -s openproxy 2>/dev/null || true
      createdb -O openproxy openproxy 2>/dev/null || warn "Could not create database — do it manually"
    fi
  else
    warn "PostgreSQL not running — start it, or spin one up with Docker:"
    echo ""
    echo "       docker run -d --name openproxy-db \\"
    echo "         -e POSTGRES_USER=openproxy \\"
    echo "         -e POSTGRES_PASSWORD=${DB_PASSWORD} \\"
    echo "         -e POSTGRES_DB=openproxy \\"
...
setup_uninstall() {
  ...
  systemctl stop  openproxy    2>/dev/null && info "Stopped openproxy"  || true
  systemctl stop  wg-quick@wg0 2>/dev/null && info "Stopped WireGuard" || true
  systemctl disable openproxy    2>/dev/null || true
  systemctl disable wg-quick@wg0 2>/dev/null || true

  step "Removing files"
  rm -f  /etc/systemd/system/openportal.service
  rm -f  /etc/nginx/sites-enabled/openproxy
  rm -f  /etc/nginx/sites-available/openproxy
  rm -f  /etc/nginx/conf.d/openproxy.conf
  rm -f  /etc/wireguard/wg0.conf
  rm -f  /etc/sysctl.d/99-openproxy-wg.conf
  ...
  if confirm "Also drop the PostgreSQL database and user?"; then
    sudo -u postgres psql -c "DROP DATABASE IF EXISTS openproxy;" 2>/dev/null || true
    sudo -u postgres psql -c "DROP USER IF EXISTS openproxy;" 2>/dev/null || true
...
print_summary() {
  ...
  case "$1" in
    ...
    bare-metal)
      ...
      echo ""
      echo -e "  ${DIM}Manage:${NC}"
      echo "    systemctl status openproxy    # service status"
      echo "    journalctl -u openproxy -f    # live logs"
      echo "    systemctl restart openproxy   # restart"
      ;;
    dev)
...
preflight() {
  if [[ ! -f "$REPO_ROOT/docker-compose.yml" ]]; then
    error "Cannot find docker-compose.yml (looked in: $REPO_ROOT). Run setup.sh from the openproxy repo root."
  fi
